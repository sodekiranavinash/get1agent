...
class FakeStorage:
    ...
    def put_json_conditional(self, key: str, value: Any, etag: str | None) -> str:
        if etag is not None and self.etags.get(key) != etag:
            from core.storage import PreconditionFailed
...
def _local_store(fake: FakeStorage):
    from retrieval.s3_vectors import LocalVectorStore
...
def patch_pipeline(monkeypatch, fake: FakeStorage) -> None:
    """Point the ingestion index stage's vector store at the in-memory double."""
    import ingestion.pipeline as index

    monkeypatch.setattr(index, "vector_store", lambda storage: _local_store(fake))


def patch_actions(monkeypatch, fake: FakeStorage) -> None:
    """Point the shared ingestion ``_storage`` helper at the in-memory double."""
    import ingestion.actions as shared_ingestion

    monkeypatch.setattr(shared_ingestion, "Storage", lambda: fake)
