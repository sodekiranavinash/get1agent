...
CI split: **Infra** (`.github/workflows/infra.yml`) creates the function, IAM, and logs. **Tool code** (`.github/workflows/tools-challan-extractor.yml`) tests, zips, and `UpdateFunctionCode`. Terraform ignores zip hash so infra apply does not overwrite a tools deploy. Add the next tool as another `tools-*.yml` caller of `deploy-tool-lambda.yml` (UI / backend / agents stay on their own workflows).
